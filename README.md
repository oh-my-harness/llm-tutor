...
| Tutor | 持久导师身份、Markdown Soul、不可变会话绑定、默认模型、能力与资料权限、导师管理。 |

“辅导机器人”现在支持创建和管理持久导师。每位导师通过 Markdown Soul 定义稳定身份与教学方式，并可配置默认模型、能力、知识库、Notebook、Space 与学习者记忆权限；独立 Tutor Memory、连续性工作区和导师交接仍在后续阶段。原 Books 能力已经移除，Research 报告只保存到 Notebook。
...
- 辅导机器人已支持持久身份、Soul、默认模型和服务端资料权限；私有 Tutor Memory、连续性工作区和导师交接尚未实现。
