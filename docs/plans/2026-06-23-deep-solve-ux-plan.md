...
- [ ] Group RAG citations by stage or final answer.
- [ ] Show source snippets below the final answer and inside evidence details.
- [ ] Show code execution results in the verification stage.
...
- [ ] Add UI affordance to ask about a specific step.
- [ ] Send follow-up metadata with `step_id` and session ID.
- [ ] Add agent prompt context for step-specific follow-up.
...
- Should Deep Solve final answer be a normal assistant message plus attached structure, or a custom message type rendered by the UI?
- Should stage summaries be generated by the LLM, derived from trace events, or both?
- Should stage details be persisted forever, or compacted into summary after a threshold?
- How much of the final answer schema should be enforced by an answer tool versus prompt convention?
