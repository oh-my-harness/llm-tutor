...
- [ ] Add source target parsing for `chat`, `notebook`, `quiz`, `research`, `book`, `kb`, and `web`.
- [ ] Add a shared navigation callback or router helper.
- [ ] Implement chat session navigation first.
- [ ] Implement Notebook entry navigation.
- [ ] Implement Quiz Bank item/question navigation.
- [ ] Implement Knowledge Base document/chunk navigation where current UI supports it.
