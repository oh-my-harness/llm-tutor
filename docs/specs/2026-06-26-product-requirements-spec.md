...
- REQ-324: Research mode shall provide a detailed research workflow that can be explicitly started after the user's need is clear. The workflow shall cover search, source reading, source selection, synthesis, citation checking, and report generation; it shall not be forced for every Research message. Status: implemented for the first runtime workflow slice; remaining hardening covers richer source metadata restore, quality scoring, regeneration, and stop/cancel behavior.
